...
class NumberToWordsConverter {
    ...
    private static handleDecimal(num: number, language: ENUM_LANGUAGE): string {
        ...
        const decimalDigits = decPart.split('').map(d => {
            const digit = parseInt(d);
            if (digit === 0) {
                return language === ENUM_LANGUAGE.AR ? 'صفر' :
                    language === ENUM_LANGUAGE.FR ? 'zéro' : 'zero';
            }
            return language === ENUM_LANGUAGE.AR ? data.ones[digit] : data.ones[digit];
        }).join(' ');
    ...
    private static convertWithScales(num: number, language: ENUM_LANGUAGE): string {
        const data = this.WORD_DATA[language];
        ...
        for (let i = scales.length - 1; i > 0; i--) {
            ...
            if (num >= divisor) {
                ...
                const remainderWords = remainder ? this.convertInteger(remainder, language) : '';
                ...
                if (language === ENUM_LANGUAGE.AR) {
                    ...
                    return quotientWords + ' ' + scaleWord + (remainderWords ? ' ' + remainderWords : '');
                }
            }
        }
    ...
    private static getArabicScaleWords(quotient: number, scaleIndex: number): string {
        const data = this.WORD_DATA[ENUM_LANGUAGE.AR];
        ...
        if (scaleIndex === 1) { // Thousands
            ...
            return this.convertArabicInteger(quotient) + ' ' + data.scales[scaleIndex];
        }
    }

    private static getScaleWord(quotient: number, scale: string, language: ENUM_LANGUAGE): string {
...
export const toEnglish = (num: number): string => toWords(num, ENUM_LANGUAGE.EN);
